...
class UDPTrackerClient(BaseTrackerClient):
    ...
    async def announce(self, server_port: int, event: EventType):
        ...
        try:
            ...
            print(f"Local IP being sent to the server: {local_ip}")
            ...
            peers_start = 20
            compact_peer_list = response[peers_start:]
            peers = []
            for i in range(0, len(compact_peer_list), 6):
                ip_bytes = compact_peer_list[i:i + 4]
                port_bytes = compact_peer_list[i + 4:i + 6]
